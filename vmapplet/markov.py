...
class MarkovModel:
    ...
    def __post_init__(self):
        ...
        assert self.transition_probabilities.shape == (no_states, no_states)
        assert len(self.occupancy_distributions) == no_states - 1  # excluding final state
...
class Markov:
    ...
    def _markov(
        ...
        state: int = np.flatnonzero(self._rng.multinomial(1, model.initial_probabilities)).item()

        states = [state]
        while True:
            state = np.flatnonzero(self._rng.multinomial(1, model.transition_probabilities[state])).item()
        ...
        for state in states:
            ...
            if fn is None:
                raise ValueError(f"Distribution {distribution['distribution']} not supported")
            while True:
        ...
        for state, occupancy in zip(states, occupancies):
            ...
            for i in range(occupancy):
                observation: int = np.flatnonzero(self._rng.multinomial(1, distribution)).item()
    ...
    def _generate_sequence(
        ...
        if sequence is None:
            raise ValueError(f"Maximum iteration of {iterations} reached in sequence generation")
        if iterations > 100:
            print(f"Warning: Number of iterations in sequence generation very large: {iterations}")
