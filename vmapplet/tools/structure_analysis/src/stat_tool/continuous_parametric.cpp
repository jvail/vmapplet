...

{
  ...
  case VON_MISES : {
    ...
    mean_resultant_length = boost::math::cyl_bessel_i(1 , dispersion) / boost::math::cyl_bessel_i(0 , dispersion);
      ...
      case DEGREE : {
        ...
        for (i = 0;i < VON_MISES_NB_STEP;i++) {
          frequency[i] = exp(dispersion * cos((value - location) * M_PI / 180)) * histo_scale /
                         (360 * boost::math::cyl_bessel_i(0 , dispersion));
          value += step;
        }
        break;
      }
      ...
      case RADIAN : {
        ...
        for (i = 0;i < VON_MISES_NB_STEP;i++) {
          frequency[i] = exp(dispersion * cos(value - location)) * histo_scale /
                         (2 * M_PI * boost::math::cyl_bessel_i(0 , dispersion));
          value += step;
        }
        break;
      }
...
//  if (dispersion < VON_MISES_CONCENTRATION_THRESHOLD) {
    ...
    case DEGREE : {
      ...
      mass = mass * step / (360 * boost::math::cyl_bessel_i(0 , dispersion));
      break;
    }
    ...
    case RADIAN : {
      ...
      mass = mass * step / (2 * M_PI * boost::math::cyl_bessel_i(0 , dispersion));
      break;
    }
    }
/*  }
