...
class output(object):
    ...
    def __init__(
        self,
        init_date=datetime.datetime(1994, 1, 1),
        frequency=1.0,
        filename=None,
        ext=".csv",
    ):
    ...
    def advance(self, dt):
        ...
        """
        if type(dt) is not datetime.timedelta:
            dt = datetime.timedelta(dt)
        self.elapsed += dt
        if self.elapsed.days >= self.period:
            self.elapsed = datetime.timedelta(0.0)
            return True
        else:
            return False
...
class counts(output):
    ...
    def __init__(
        ...
        output.__init__(
            self, init_date=init_date, frequency=frequency, filename=filename, ext=ext
        )
    ...
    def save(self, date):
        ...
        self.file.write(
            "{date}, {days}, {metamers}, {gus}, {leaves}, {tla}, {fruits}, {fruitdw}\n".format(
                date=date,
                days=(date - self.init_date).days,
                metamers=self.metamers,
                gus=self.gus,
                leaves=self.leaves,
                tla=self.tla,
                fruits=self.fruits,
                fruitdw=self.fruitdw,
            )
        )
...
class shoots(output):
    ...
    def openfile(self):
        output.openfile(self)
        self.file.write(
            "Date, Florals, Shorts, Mediums, All_Longs, Longs_16_to_25, Longs_26_to_40, Longs_over_40\n"
        )
    ...
    def save(self, date):
        ...
        self.file.write(
            "{date}, {florals}, {shorts}, {mediums}, {longs}, {longshort}, {longmedium}, {longlong}\n".format(
                date=date,
                florals=self.florals,
                shorts=self.shorts,
                mediums=self.mediums,
                longs=self.longs,
                longshort=self.len_16_to_25,
                longmedium=self.len_26_to_40,
                longlong=self.len_over_40,
            )
        )
...
class sequences(output):
    ...
    def openfile(self):
        output.openfile(self)
        self.file.write(
            "#save sequences of each apex\n\n1 VARIABLE\n\nVARIABLE 1 : VALUE\n\n"
        )

    def save(self, sequence, position):
        for i in range(
            position - 1, -1, -1
        ):  # up to -1 (c notation so really it's up to 0)
            if i == 0:
                self.file.write("%s\n" % sequence[i][1])
            else:
...
class trunk(output):
    ...
    def save(self, date, trunk_radius, trunk_cross_sectional_area):
        ...
        self.file.write(
            "{date}, {days}, {radius}, {section_area}\n".format(
                date=date,
                days=(date - self.init_date).days,
                radius=trunk_radius,
                section_area=trunk_cross_sectional_area,
            )
        )
        ...
        for i, elt in enumerate(lstring):
            ...
            if elt.name == "growth_unit" and previous == "branch":
                ...
                if self.current_column > self.columns:
                    raise ValueError(
                        "ERROR: Not enough columns were allocated for the MTG file.\n"
                    )
                ...
                self.write_trailing_tabs()
                self.file.write(
                    "\t%.4f\t%.4f\t%.4f\t%.4f\t%4.0f\t%s\t%.4f\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%u\t%u\t%u\t%s\t%.4f\t%.4f\n"
                    % (
                        m.radius * 2000.0,
                        m.position.x,
                        m.position.y,
...
class Data(object):
    ...
    def open_all(self, simu_id):
        ...
        if self.options.output.shoots:
            self.shoots.build_filename(
                directory=self.directory,
                tag="{0}_{1}".format(self.options.general.tag, simu_id),
            )
            self.shoots.openfile()

        # init the sequence
        if self.options.output.sequences:
            self.sequences.build_filename(
                directory=self.directory,
                tag="{0}_{1}".format(self.options.general.tag, simu_id),
            )
