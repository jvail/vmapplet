...
second_moment_of_area_annular_section = cppyy.gbl.optimization.second_moment_of_area_annular_section
...
def reorient_frame(initial_hlu, rotation_velocity, rv_norm, length):
    ...
    if fabs(rv_norm*length) >= 0.01:
        h = pgl.Vector3(rotate(rotation_velocity.x, rotation_velocity.y, rotation_velocity.z, rv_norm*length, h.x, h.y, h.z))
        l = pgl.Vector3(rotate(rotation_velocity.x, rotation_velocity.y, rotation_velocity.z, rv_norm*length, l.x, l.y, l.z))
    h.normalize()
    l.normalize()
